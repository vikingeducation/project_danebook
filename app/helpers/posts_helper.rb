module PostsHelper
  ...
  def show_post_likes(item)
    ...
    if likes.where(user_id: current_user.id).any? && likes.count == 1
      ...
      str = "<div class='liked-box'>  You and <a href= '#{user_path(user)}' class='liking'>  #{user.first_name} #{user.last_name} </a> <p> and #{likes.count} others like this </p></div>"
      ...
      str = "<div class='liked-box'> <a href= '#{user_path(user)}' class='liking'>  #{user.first_name}, #{user.last_name} </a> <p> and #{likes.count} others like this </p></div>"
    end 
    str.html_safe
  end
